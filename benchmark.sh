...
EOF
            ...
            run_project="python3 src/main.py --generate"
            ...
            outdir="$(basename "$representation")"
            mkdir -p $outdir

            #Save to .csv
            echo "$(basename $representation),$instance_size,$time" >> $outdir/$(basename $routine)_benchmark_time.csv
            echo "$(basename $representation),$instance_size,$mem" >> $outdir/$(basename $routine)_benchmark_memory.csv
